...
async function buildBridgeServer(): Promise<void> {
  ...
  const prebuiltBundle = join(ROOT_DIR, "apps/electron/resources/bridge-mcp-server/index.js");
  ...
  } else if (existsSync(prebuiltBundle)) {
    // Fall back to pre-built bundle from resources (source not available in fork)
    const { copyFileSync } = await import("fs");
    copyFileSync(prebuiltBundle, BRIDGE_SERVER_OUTPUT);
    console.log("📦 Using pre-built bridge server bundle from resources");
  } else {
    console.warn("⚠️  Bridge MCP server source not found, skipping (API sources in Codex sessions will not be available)");
    return;
  }
...
async function buildSessionServer(): Promise<void> {
  ...
  const prebuiltBundle = join(ROOT_DIR, "apps/electron/resources/session-mcp-server/index.js");
  ...
  if (existsSync(sourceEntry)) {
    ...
    // Fall back to pre-built bundle from resources (source not available in fork)
    const { copyFileSync } = await import("fs");
    copyFileSync(prebuiltBundle, SESSION_SERVER_OUTPUT);
    console.log("📦 Using pre-built session server bundle from resources");
  } else {
